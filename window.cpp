...
4. 현재 경로에서 나올경우 이전경로를 이전에 저장했던 경로로 바꿔줍니다. 그리고 CString 변수인 현재 디렉터리 변수도 ""공백문자로 바꿔줍니다 경로가 계속 더해질 경우
...
사용자가 명령어를 입력하면 터미널에서 작업을 입력한 명령을 해석하여 실행합니다. 그리고 결과를 화면에 출력합니다. 그러면 출력된 결과 화면을 보고 
	다시 명령을 입력하는 대화식 작업을 수행합니다. 입력한 명령어 실행이 결과가 나올 때 까지 기다리는 방식이 바로 포그라운드 방식 입니다.  
	포그라운드 프로세스라고 말하며 작업 제어 측면에서는 포그라운드 작업이라고 말합니다. 
...
백그라운드 방식으로 명령어를 실행하면 곧바로 다음 명령어를 실행 가능합니다. 필요한 여러 작업 동시에 진행하면서 포그라운드 작업을 계속 진행할 수 있습니다.
	백그라운드 작업은 해당 명령어 처리가 오래 걸릴걸 대비해 백그라운드를 이용하면 여러 작업을 동시에 수행할 수 있다. 


SendMessage & PostMessage 이해하기
SendMessage는 윈도우 프로시저(window procedure)를 직접 호출하며, 프로시저가 메시지를 처리할 때 까지 반환하지 않습니다. 즉, 함수 내에 다른 함수가 호출되면 
	그 함수가 반환할 때 까지 기다려야 하듯이 일반 함수 호출과 동일하게 동작합니다.
PostMessage는 호출된 메시지가 메시지 큐에 들어가며, 윈도우 프로시저에서 이 메시지를 처리하게 됩니다. 이것이 의미하는 것은, 메시지가 즉각 처리되는 것이 아니라
	GetMessage()에 의해 해석된 메시지가 DispatchMessage()에 의해 윈도우 프로시저로 전달되어 처리됩니다. 그래서 PostMessage에 의해 전달된 메시지는
	언제 처리될 지 예측하기가 어렵습니다.
두 함수의 차이를 요약하자면 다음과 같습니다.


 SendMessage	PostMessage
윈도우 프로시저를 직접 호출하며, 프로시저가 메시지를 처리할 때 까지 반환하지 않는다.	 메시지 큐에 메시지가 삽입되며, 윈도우 프로시저에서 메시지를 처리한다. 
